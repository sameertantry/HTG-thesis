...
def build_encoder_from_config(config: DictConfig) -> nn.Module:
    ...
    encoder = nn.Sequential(
        ...
        nn.Conv2d(in_channels=timm_model.blocks[config.num_layers - 1][0].conv.out_channels, out_channels=config.out_channels, kernel_size=(2, 3), padding=(0, 1), bias=False),
...
class CRNN(nn.Module):
    def __init__(self, config: DictConfig):
        ...
        self.rnn = nn.Sequential(
            ...
            BidirectionalLSTM(config.rnn_hidden_size, config.rnn_hidden_size, config.num_classes)
        )
