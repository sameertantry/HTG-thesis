...
from handwritten_generation.models.modules import ConvBlock, CrossAttention, SelfAttention
...
class CrossAttentionBasedCondition(nn.Module):
    def __init__(self, context_hidden_dim: int, num_channels: int, dropout: float = 0.1):
        super().__init__()
        
        self.cross_attention = CrossAttention(hidden_dim=num_channels, context_hidden_dim=context_hidden_dim, dropout=dropout)
...
class PatchedCrossAttentionBasedCondition(CrossAttentionBasedCondition):
    def __init__(self, context_hidden_dim: int, num_channels: int, patch_size: int, dropout: float = 0.1):
...
class TimeCondition(nn.Module):
    ...
    def forward(self, x, time_emb):
        time_emb = self.proj(time_emb)[:, :, None, None].repeat(1, 1, x.size(-2), x.size(-1))

        return F.gelu(x + time_emb)


class UNet(nn.Module):
    def __init__(self, time_emb_dim: int, text_emb_dim: int, in_channels: int, out_channels: int):
        super().__init__()
        ...
        self.bottleneck = nn.Sequential(
            ...
            ConvBlock(in_channels=1024, out_channels=1024, mid_channels=2048, residual=True),
        ...
        self.text_condition = nn.ModuleList([
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=64, patch_size=8),
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=128, patch_size=4),
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=256, patch_size=2),
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=512, patch_size=2),
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=256, patch_size=4),
            PatchedCrossAttentionBasedCondition(context_hidden_dim=text_emb_dim, num_channels=128, patch_size=8),
        ])
